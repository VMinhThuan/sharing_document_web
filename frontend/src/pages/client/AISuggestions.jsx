...
import { getRecommendationsApi, toggleFavoriteApi } from "../../services/api";
...
const AISuggestions = () => {
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      setLoading(true);
      try {
        const res = await getRecommendationsApi(pageSize, currentPage);
        if (res && res.statusCode === 200) {
  ...
  return (
    <main className="flex-1 flex flex-col min-h-full bg-[#f9fafb] dark:bg-background-dark relative">
      <TopHeader title="AI Suggested For You" />
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-primary mb-2">
                ...
                <span className="text-sm font-black uppercase tracking-widest">Personalized AI Picks</span>
              </div>
              <h1 className="text-4xl font-black text-gray-900 dark:text-white tracking-tight">
                AI Suggested For You
              </h1>
              <p className="text-gray-500 dark:text-gray-400 text-lg max-w-2xl font-medium">
                Our AI analyzed your interests and behavior to find the perfect study materials just for you.
              </p>
            </div>
          ...
          {!isAuthenticated ? (
               ...
               <Empty
                description={
                  <span className="text-gray-500 font-bold">
                    Please login to see personalized AI suggestions
                  </span>
                }
              />
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="mt-4 text-gray-500 font-medium italic">
                AI is crunching parameters for you...
              </p>
            </div>
                ...
                >
                  <div className="relative aspect-[16/10] rounded-2xl overflow-hidden bg-slate-100 dark:bg-slate-800 flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900 border border-gray-50 dark:border-gray-700">
                    ...
                    <div className="absolute top-4 left-4 flex flex-col gap-2">
                      <span className="px-3 py-1 rounded-lg bg-white/95 dark:bg-black/60 backdrop-blur-md text-[10px] font-black text-gray-700 dark:text-gray-200 shadow-sm uppercase tracking-widest border border-white/20 text-center">
                        {formatFileType(doc.fileType)}
                      </span>
                      {doc._matchPercentage && (
                        <span className="px-3 py-1 rounded-lg bg-primary/95 backdrop-blur-md text-[10px] font-black text-white shadow-sm uppercase tracking-widest border border-white/20 flex items-center gap-1">
            ...
            <div className="py-20 text-center bg-white dark:bg-[#1a202c] rounded-3xl border border-dashed border-gray-300 dark:border-gray-700">
              <Empty
                description={
                  <span className="text-gray-500 font-bold">
                    No recommendations found. Try updating your interests in Preferences!
                  </span>
                }
              />
            </div>
